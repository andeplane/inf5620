...
\begin{align*}
  \int_\Omega(\vec u^* - \vec u^{n})\cdot\vec v^{(u)}\dm \Omega
  &- \int_\Omega\Delta t\nu\nabla^2\vec u^n\cdot\vec v^{(u)} \dm \Omega\\
   &+ \int_\Omega\beta\frac{\Delta t}{\rho}\nabla p^n \cdot\vec v^{(u)} \dm \Omega 
   - \int_\Omega\Delta t\vec f\cdot\vec v^{(u)} \dm \Omega 
   + \int_\Omega\Delta t\vec u^n\cdot \nabla\vec u^n\cdot\vec v^{(u)} \dm \Omega.
\end{align*}
By integrating the Laplace term and the pressure term by parts, we get
\begin{align*}
  \int_\Omega(\vec u^* - \vec u^{n})\cdot\vec v^{(u)} &+ \Delta t\nu\nabla\vec u^n\cdot\nabla\vec v^{(u)} - \beta\frac{\Delta t}{\rho}p^n \nabla\cdot\vec v^{(u)}\dm \Omega\\
   & - \int_\Omega\Delta t\vec f\cdot\vec v^{(u)} \dm \Omega 
   + \int_\Omega\Delta t\vec u^n\cdot \nabla\vec u^n\cdot\vec v^{(u)} \dm \Omega - \int_{\partial\Omega_{N,u}} (\nabla \vec u^n - p\vec n)\cdot \vec v^{(u)} \dm s
\end{align*}
...
\begin{align*}
  ...
  \int_\Omega \nabla\Phi \cdot \nabla v^{(\Phi)} &= \frac{\rho}{\Delta t} \int_\Omega \nabla \cdot \vec u^* v^{(\Phi)} \dm \Omega
\end{align*}
\end{document}
