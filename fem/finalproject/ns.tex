...
\begin{align*}
  \dpart{\vec u}{t} = \nu\nabla^2\vec u - \frac{1}{\rho}\nabla p \vec u + \vec f - \vec u\cdot \nabla\vec u,
\end{align*}
where we have defined the kinematic viscosity $\nu=\mu/\rho$, redifined $\vec f \rightarrow \vec f/\rho$, force per unit volume.
\subsection{Forward Euler}
The simplest approach is to apply an explicit scheme for the time derivative
\begin{align*}
  \frac{\vec u^{n+1} - \vec u^{n}}{\Delta t} = \nu\nabla^2\vec u^n - \frac{1}{\rho}\nabla p \vec u^n + \vec f - \vec u^n\cdot \nabla\vec u^n,
\end{align*}
which we can solve directly for $\vec u^{n+1}$
\begin{align*}
  \vec u^{n+1} = \vec u^{n} + \Delta t\nu\nabla^2\vec u^n - \frac{\Delta t}{\rho}\nabla p^n \vec u^n + \Delta t\vec f - \Delta t\vec u^n\cdot \nabla\vec u^n.
\end{align*}
With this scheme, we haven't used the fact that $\nabla \cdot \vec u=0$ at all times, so we don't know that $\vec u^{n+1}$ fulfills this requirement. In addition, the pressure $p$ is constant in time. 
\subsection{A working scheme}
We can create some degrees of freedom by using the forward euler scheme, but evaluating the pressure at time step $n+1$
\begin{align*}
  \vec u^{n+1} = \vec u^{n} + \Delta t\nu\nabla^2\vec u^n - \frac{\Delta t}{\rho}\nabla p^{n+1} \vec u^n + \Delta t\vec f - \Delta t\vec u^n\cdot \nabla\vec u^n.
\end{align*}
...
\end{align*}
By using this on our expression for $\delta \vec u$
\begin{align*}
  \nabla^2 \Phi = \frac{\rho}{\Delta t}\nabla \cdot \vec u^*. 
\end{align*}
This is simply a Poisson equation for $\Phi$ that can be solved since we know $\vec u^*$. After finding $\Phi$, we find
\begin{align*}
